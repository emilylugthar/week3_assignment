...
# garden_harvest %>% 
#   left_join(left_join(plant_min_date
#                       by = c("vegitable", "variety")))
```
Some vegetables, like Garden Party radishes and most of the tomato varieties were planted in different plots on different dates. You could try creating a dataset that only includes plots and plantings on the first date, but the resulting dataset would still not give you an accurate sense the amount of produced produced by each plot.
...
garden_harvest %>% 
  ...
  mutate(stringlength = str_length(variety)) %>% 
...
Trips %>% 
  ...
  mutate(time_of_day = hour + minute) %>% 
  mutate(day = wday(sdate, label = TRUE)) %>% 
  mutate(weekday = ifelse( day == "Sat" | day == "Sun", "weekend", "weekday")) %>% 
  ggplot(aes(time_of_day)) +
